...
def load_cifar(noise):
    ...
    test_data = np.array(test_images_[:750], dtype=np.float32) / 255
    test_labels = np.array(train_labels[:750], dtype=np.int32)
...
def main(unused_argv):
  ...
  for i in [.792]:
    for iteration in range(1,11):
        #eps = compute_epsilon(FLAGS.epochs * FLAGS.size // FLAGS.batch_size,i)
        #print('For delta=%4f, the current epsilon is: %.4f' % (FLAGS.delta, eps))
