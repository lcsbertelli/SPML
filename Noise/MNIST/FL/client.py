...
flags.DEFINE_integer('startsize', startS, 'subset for training')
flags.DEFINE_integer('endsize', endS, 'subset for training')
flags.DEFINE_integer('size', 2500, 'subset for training')
...
def load_mnist(noise):
  ...
  train_data = np.array(train_data[FLAGS.startsize:FLAGS.endsize], dtype=np.float32) / 255
  test_data = np.array(test_data[:2500], dtype=np.float32) / 255
  train_data = train_data.reshape(train_data.shape[0], 28, 28, 1)

  test_data = test_data.reshape(test_data.shape[0], 28, 28, 1)

  train_labels = np.array(train_labels[FLAGS.startsize:FLAGS.endsize], dtype=np.int32)
...
def training(noise):
    ...
    if path.exists("aggModel.h5"):
        print("Agg model exists...\nLoading model...")
        model = load_model("aggModel.h5")
    else:
        # Define a sequential Keras model
        print("Agg model don't exists...\nTraining model...")
...
def main(unused_argv):
  ...
  #for i in [30,4.2,2.35,1.42,1.1,.930]:
  for i in [.729]:
#for i in [30]:
