    ...
    for epoch in range(initial_epoch, epochs):
        ...
        if steps_per_epoch is not None:
            ...
            for batch_index, (batch_start, batch_end) in enumerate(batches):
                ...
                #print("batch_logs",batch_logs)
                callbacks._call_batch_hook('train', 'begin', batch_index, batch_logs)

                #print("going to indices",indices_for_conversion_to_dense)
                for i in indices_for_conversion_to_dense:
                    ins_batch[i] = ins_batch[i].toarray()

                #print("ins in training_array.py",ins_batch)
...
def test_loop(model, f, ins,
    ...
    if steps is not None:
        ...
        for batch_index, (batch_start, batch_end) in enumerate(batches):
            ...
            batch_outs = f(ins_batch)
            #batch_outs = np.mean(f(ins_batch)[0]).tolist()
            #print("type",type(batch_outs))
            #print("batch_outs",batch_outs)
            if isinstance(batch_outs, list):
                if batch_index == 0:
                    outs.extend([0.] * len(batch_outs))
                for i, batch_out in enumerate(batch_outs):
                    if i == 0:  # Index 0 == `Loss`
                        ...
                        #print("batch_outs",type(batch_out))
                        #print("length",batch_ids)
                        x=batch_out * len(batch_ids)
                        #print("x",x)
                        #print("Now")
                        ...
                        #print("in else of i=0")
                        #print("batch_out",batch_out)
                        #print("batch_outs",type(batch_out))
                        #print("length",len(batch_ids))
                        #x=float(batch_out) * len(batch_ids)
                        #print("x",x)
                        #print("Now")
                        outs[i] = float(batch_out)
                #print("output",outs)
